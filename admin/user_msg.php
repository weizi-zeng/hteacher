...
//-- 列出所有$from的留言
/*------------------------------------------------------ */
if (strpos($_REQUEST['act'], "list_")>-1)
{
	assign_query_info();
	
	$from = substr($_REQUEST['act'], strpos($_REQUEST['act'], "list_")+5);
	$msg_list = msg_list($from);
	...
	$smarty->assign('list_from',     $_REQUEST['act']);
...
if ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        $link[] = array('text' => $_LANG['back_list'], 'href' => 'user_msg.php?act='.$_POST['list_from']);
        sys_msg(sprintf("操作成功，%d项数据发生改变", count($_POST['checkboxes'])), 0, $link);
    }
    else
    {
        /* 提示信息 */
        $link[] = array('text' => $_LANG['back_list'], 'href' => 'user_msg.php?act='.$_POST['list_from']);
        sys_msg("请选择您需要操作的项", 0, $link);
    }
}
...
elseif ($_REQUEST['act']=='view')
{
    ...
    $smarty->assign('action_link', array('text' => "系统留言列表", 'href'=>'user_msg.php?act='.$_REQUEST['list_from']));
