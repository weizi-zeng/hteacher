...
if ($_REQUEST['act'] == 'list')
{
    ...
    $classAdmin_list = classAdmin_list();
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if (!empty($_POST['email']))
    {
     ...
     $sql = "INSERT INTO ".$ecs->table('admin_user')." (user_name, name, email, cellphone, password, add_time, nav_list, action_list, role_id, status_id, school_code, creator_id) ".
               "VALUES ('".trim($_POST['user_name'])."', '".trim($_POST['name'])."','".trim($_POST['email'])."', '".trim($_POST['cellphone'])."','$password', '$add_time', '', 'all', '0', '1', '".$code."','".$_SESSION["admin_id"]."')";
...
function classAdmin_list()
{
    ...
    if ($result === false)
    {
        ...
        $filter['school_code'] = empty($_REQUEST['school_code']) ? '' : trim($_REQUEST['school_code']);//学校
        $filter['class_code'] = empty($_REQUEST['class_code']) ? '' : trim($_REQUEST['class_code']);//班级
        $filter['creator_id'] = empty($_REQUEST['creator_id']) ? '' : trim($_REQUEST['creator_id']);//创建人
        ...
        $ex_where = ' WHERE status_id=2  ';//班级管理员 status_id=2
        if ($filter['keywords'])
        {
