...
/* 初始化 $exc 对象 */
$exc = new exchange($table, $db, 'user_id', 'user_name');
...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    $sql="SELECT `ec_salt` FROM ". $table ."WHERE user_name = '" . $_POST['username']."'";
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if (!empty($_POST['select_role']))
    {
        ...
        $sql = "SELECT nav_list FROM " . $table . " WHERE action_list = 'all'";
        $row = $db->getRow($sql);


    $sql = "INSERT INTO ".$table." (user_name, email, password, add_time, nav_list, action_list, role_id) ".
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取管理员信息 */
    $sql = "SELECT user_id, user_name, email, password, agency_id, role_id FROM " .$table.
    ...
    /* 获得该管理员的权限 */
    $priv_str = $db->getOne("SELECT action_list FROM " .$table. " WHERE user_id = '$_GET[id]'");
...
elseif ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self')
{
    ...
    {
        /* 查询旧密码并与输入的旧密码比较是否相同 */
        $sql = "SELECT password FROM ".$table." WHERE user_id = '$admin_id'";
        $old_password = $db->getOne($sql);
		$sql ="SELECT ec_salt FROM ".$table." WHERE user_id = '$admin_id'";
...
elseif ($_REQUEST['act'] == 'modif')
{
    ...
    $sql = "SELECT user_id, user_name, email, nav_list ".
           "FROM " .$table. " WHERE user_id = '".$_SESSION['admin_id']."'";
    $user_info = $db->getRow($sql);
...
elseif ($_REQUEST['act'] == 'update_allot')
{
    ...
    /* 取得当前管理员用户名 */
    $admin_name = $db->getOne("SELECT user_name FROM " .$table. " WHERE user_id = '$_POST[id]'");

    /* 更新管理员的权限 */
    $act_list = @join(",", $_POST['action_code']);
    $sql = "UPDATE " .$table. " SET action_list = '$act_list', role_id = '' ".
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 获得管理员用户名 */
    $admin_name = $db->getOne('SELECT user_name FROM '.$table." WHERE user_id='$id'");
