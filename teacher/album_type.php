...
{
	$smarty->display('album_type_list.htm');
	exit;
}

if ($_REQUEST['act'] == 'ajax_list')
{
	$list = album_type_list();
	make_json($list);
}
...
elseif ($_REQUEST['act'] == 'ajax_save')
{
	...
	//判断重复
	$isExit = $db->getRow("select * from ".$ecs->table("album_type")." where name='".$_REQUEST["name"]."' and class_code='".$_SESSION["class_code"]."' and atype_id!=".$id." limit 1 ");
	if($isExit){
		make_json_error("类型“".$_REQUEST["name"]."”已经存在！ID为：“".$isExit["atype_id"]."”");
		exit;
	}
	
	if($id==0){//insert
		
		$sql = "insert into ".$ecs->table("album_type")
		...
		admin_log(addslashes($_REQUEST["name"]), 'add', 'album_type');
		make_json_result("添加“".$_REQUEST["name"]."”成功！");
	}
	
	else //update
	{
		$sql = "update ".$ecs->table("album_type")
...
function album_type_list()
{
	...
	if ($result === false)
	{
		...
		$filter['sort']    = empty($_REQUEST['sort'])    ? 'atype_id' : trim($_REQUEST['sort']);
		...
		$sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table("album_type") . $ex_where;
		...
		$sql = "SELECT * ".
                " FROM " . $GLOBALS['ecs']->table("album_type")  . $ex_where .
