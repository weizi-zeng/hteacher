  var me = {
    ...
    function deletemessage() {
        ...
        $.messager.confirm('提示信息', '确认删除将会将本条消息以及相关的回复全部删除，确认要删除选择项？【'+ids+ ','+ name + '】', function (isClickedOk) {
            if (isClickedOk) {
                $.ajax({
    ...
    function getedMessage(){
    	$('#dgData').datagrid('load', {
    		search_to_type: 'guardian',
    		search_to_: guardian_id
    	});
    }
 
    function sendedMessage(){
    	$('#dgData').datagrid('load', {
    		search_from_type: 'guardian',
    		search_from_: guardian_id
    	});
    }